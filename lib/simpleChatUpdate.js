...
exports.chatUpdate = async(conn, m) => {
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.id = m.smessage.contextInfo.stanzaId
            m.quoted.chat = m.smessage.contextInfo.remoteJid || m.chat
            m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('3EB0') && m.quoted.id.length === 12 : false
            m.quoted.sender = m.smessage.contextInfo.participant
      ...
      if (m.smessage.url) m.download = (type = 'buffer') => conn.downloadM(m, type)

      m.text = (m.type == 'listResponseMessage' ? m.smessage.singleSelectReply.selectedRowId : '') || m.smessage.text || m.smessage.caption || m.smessage || ''
